...
from policy.transforms.canonicalization.no_proprio_vel import NoProprioVel
...
class TestRemoveProprioVel:
    ...
    def test_tensor_remove_slice(self):
        rp = NoProprioVel(qpos_dim=9, qvel_dim=9, fill_with_zeroes=False)
    ...
    def test_tensor_zero_slice(self):
        rp = NoProprioVel(qpos_dim=9, qvel_dim=9, fill_with_zeroes=True)
        state = torch.randn(2, 30)
        out = rp(state)
    ...
    def test_dict_agent_qvel_remove(self):
        rp = NoProprioVel(fill_with_zeroes=False)
    ...
    def test_dict_agent_qvel_zeroed(self):
        rp = NoProprioVel(fill_with_zeroes=True)
        obs = {"agent": {"qpos": torch.randn(9), "qvel": torch.randn(9)}, "extra": torch.randn(3)}
        out = rp(obs)
    ...
    def test_custom_dims(self):
        rp = NoProprioVel(qpos_dim=7, qvel_dim=7, fill_with_zeroes=False)
