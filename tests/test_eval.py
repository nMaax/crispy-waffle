...
def test_eval_rollout_callback_is_self_contained(
    ...
    rollout_cb = next(
        cb for cb in trainer.callbacks if isinstance(cb, RolloutEvaluationCallback)
    )
    ...
    rollout_cb.setup(
        trainer=Mock(datamodule=None), pl_module=Mock(obs_horizon=2), stage="test"
    )
