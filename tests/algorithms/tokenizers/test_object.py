...
class TestObjectTokenizer:
    def test_output_dim_relative_and_absolute_modes(self):
        assert ObjectTokenizer(relative_goal=True).output_dim == 9
        assert ObjectTokenizer(relative_goal=False).output_dim == 10
    ...
    def test_enriched_token_structure(self):
        ...
        # 2 objects: obj_0_pose, obj_1_pose (tcp_pose is not tokenized)
        assert tokens.shape == (1, 1, 2, 9)

        # Token 0 (obj_0, pick):
        # 1. goal_delta: goal_0 (1,2,3) - obj_0 (0,0,0) = (1, 2, 3), rotvec pi/2 about z
        tok_0 = tokens[0, 0, 0]
        ...
        # 2. role: [1, 0, 0] (pick)
        assert torch.allclose(tok_0[6:9], torch.tensor([1.0, 0.0, 0.0]))

        # Token 1 (obj_1, target):
        # 1. goal_delta: goal_1 == obj_1 -> 0
        tok_1 = tokens[0, 0, 1]
        assert torch.allclose(tok_1[:6], torch.zeros(6), atol=1e-6)
        # 2. role: [0, 1, 0] (target)
        assert torch.allclose(tok_1[6:9], torch.tensor([0.0, 1.0, 0.0]))
    ...
    def test_dynamic_clutter_tokens(self):
        ...
        tokens = tokenizer.tokenize(obs, goal)
        assert tokens.shape == (1, 1, 4, 9)  # obj_0, obj_1, obj_2, obj_3

        # Check clutter roles: [0, 0, 1]
        assert torch.allclose(tokens[0, 0, 2, 6:9], torch.tensor([0.0, 0.0, 1.0]))
        assert torch.allclose(tokens[0, 0, 3, 6:9], torch.tensor([0.0, 0.0, 1.0]))
    ...
    def test_single_side_tokenization_absolute_mode(self):
        ...
        assert tokens.shape == (1, 1, 2, 10)
        # Check raw pose in token[0:7] matches obs["obj_0_pose"]
        assert torch.equal(tokens[0, 0, 0, 0:7], obs["obj_0_pose"][0, 0])
        assert torch.equal(tokens[0, 0, 1, 0:7], obs["obj_1_pose"][0, 0])
    ...
    def test_natural_obj_slots_with_role_tensors(self):
        ...
        tokens = tokenizer.tokenize(obs, goal)
        assert tokens.shape == (1, 1, 4, 9)

        # Slot 0 (clutter): role [0, 0, 1]
        assert torch.allclose(tokens[0, 0, 0, 6:9], torch.tensor([0.0, 0.0, 1.0]))
        # Slot 1 (clutter): role [0, 0, 1]
        assert torch.allclose(tokens[0, 0, 1, 6:9], torch.tensor([0.0, 0.0, 1.0]))
        # Slot 2 (pick): role [1, 0, 0], goal delta to target
        assert torch.allclose(tokens[0, 0, 2, 6:9], torch.tensor([1.0, 0.0, 0.0]))
        assert torch.allclose(tokens[0, 0, 2, 0:3], torch.tensor([1.0, 0.0, 0.05]))
        # Slot 3 (target): role [0, 1, 0], goal delta 0
        assert torch.allclose(tokens[0, 0, 3, 6:9], torch.tensor([0.0, 1.0, 0.0]))
        assert torch.allclose(tokens[0, 0, 3, 0:6], torch.zeros(6), atol=1e-6)
