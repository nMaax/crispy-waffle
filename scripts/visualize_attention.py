...
def build_metadata_str(
    ...
    replayed episode and its (fixed) goal frame, tokenizer/embedder/pooling architecture, goal-
    ...
    `episode_key`/`goal_frame_idx` are runtime facts of this particular invocation, not the
    checkpoint's own config, so they're kept out of `_describe_model_config` (shared with the
    filename slug) and added here directly.
    """
    fields = [
        ("env", env_id),
        ("episode", episode_key),
        ("goal_frame", f"{goal_frame_idx}/{seq_len - 1}"),
...
def main() -> None:
    ...
    metadata_str = build_metadata_str(model, cfg, env_id, target_key, goal_frame_idx, seq_len)
