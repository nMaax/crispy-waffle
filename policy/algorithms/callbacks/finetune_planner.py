...
class FinetunePlannerCallback(BaseFinetuning):
    """Finetuning callback that freezes a backbone network and keeps an planner module trainable,
    then unfreezes the backbone network at a specified global step with a custom learning rate."""
    ...
    def __init__(
        ...
        *,
        ...
        super().__init__()
        self.unfreeze_step = (
            unet_unfreeze_step if unet_unfreeze_step is not None else unfreeze_step
        )
        self.backbone_lr = unet_lr if unet_lr is not None else backbone_lr
        self.backbone_attr = backbone_attr
        self.planner_attr = planner_attr
    ...
    def freeze_before_training(self, pl_module):
        ...
        planner = self._get_module_attr(pl_module, self.planner_attr)
        self.freeze(backbone)
        self.make_trainable(planner)
