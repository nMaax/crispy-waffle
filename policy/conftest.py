...
For example, when running `python policy/main.py algorithm=image_classifier`.
...
flowchart TD
datamodule_config[
    <a href="#policy.conftest.datamodule_config">datamodule_config</a>
] -- 'datamodule=A' --> command_line_arguments
algorithm_config[
    <a href="#policy.conftest.algorithm_config">algorithm_config</a>
] -- 'algorithm=B' --> command_line_arguments
command_line_overrides[
    <a href="#policy.conftest.command_line_overrides">command_line_overrides</a>
] -- 'seed=123' --> command_line_arguments
command_line_arguments[
    <a href="#policy.conftest.command_line_arguments">command_line_arguments</a>
] -- load configs for 'datamodule=A algorithm=B seed=123' --> dict_config
dict_config[
    <a href="#policy.conftest.dict_config">dict_config</a>
] -- instantiate objects from configs --> config
config[
    <a href="#policy.conftest.config">config</a>
] --> datamodule & algorithm
datamodule[
    <a href="#policy.conftest.datamodule">datamodule</a>
] --> algorithm
algorithm[
    <a href="#policy.conftest.algorithm">algorithm</a>
...
if typing.TYPE_CHECKING:
    ...
    from policy.trainers.jax_trainer import JaxTrainer
...
def algorithm_config(request: pytest.FixtureRequest) -> str | None:
    ...
    included tests, for example as is done in [policy.algorithms.image_classifier_test][].
    """
...
def dict_config(
    ...
    logger.info(
        ...
        + "\n"
        + "```\n"
        + ("python policy/main.py " + " ".join(command_line_arguments) + "\n")
        + "```\n"
    )
...
def setup_with_overrides(
    ...
    def test_something(dict_config: omegaconf.DictConfig):
        ...
        assert dict_config["algorithm"]["_target_"] == "policy.algorithm.image_classifier.ImageClassifier"
