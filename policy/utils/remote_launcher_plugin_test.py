...
# Use monkeypatch.setattr(policy.utils.remote_launcher_plugin, ..., that_mock)
...
import policy.main
import policy.utils.remote_launcher_plugin
from policy.main import PROJECT_NAME, main
from policy.main_test import CONFIG_DIR
from policy.utils import remote_launcher_plugin
from policy.utils.env_vars import SLURM_JOB_ID
from policy.utils.remote_launcher_plugin import RemoteSlurmLauncher
from policy.utils.testutils import IN_GITHUB_CI, IN_SELF_HOSTED_GITHUB_CI
...
def test_run_remote_job(argv: list[str], monkeypatch: pytest.MonkeyPatch):
    ...
    monkeypatch.setattr(
        policy.utils.remote_launcher_plugin, RemoteSlurmLauncher.__name__, launcher_mock
    )

    monkeypatch.setattr(sys, "argv", ["policy/main.py"] + argv)
