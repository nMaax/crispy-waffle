import torch
from torch.utils.data import Dataset

from policy.utils.typing_utils import AdapterProtocol, TensorTree

from .trajectory_dataset import TrajectoryDataset


class AlignedStatesDataset(Dataset):
    """Generates training pairs by converting raw environment observations into a target domain
    using a specified adapter."""

    def __init__(self, base_dataset: TrajectoryDataset, adapter: AdapterProtocol):
        if not isinstance(adapter, AdapterProtocol):
            raise ValueError(
                "Adapter must be an instance matching AdapterProtocol."
            )

        self.base_dataset = base_dataset
        self.adapter = adapter

    def __len__(self) -> int:
        return len(self.base_dataset)

    def __getitem__(self, idx: int) -> tuple[TensorTree, TensorTree]:
        batch = self.base_dataset[idx]

        x = batch["obs_seq"]

        with torch.no_grad():
            y = self.adapter.apply(x)

        return x, y
